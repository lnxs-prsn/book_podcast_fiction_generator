...
process_state_summary:
  op_what_is_missing: { touch: 1, last_chapter: "001" }
  op_identify_unknown: "owned"
  op_separate_condition: { touch: 1, last_chapter: "004" }
  op_look_at_unknown: { touch: 1, last_chapter: "005" }

macro_mystery_evidence:
  - chapter: "001" — "The same figure appeared on the street before the gate opened and afterward recorded that the empty niche stayed visually constant across three different solver approaches, as if the gate were tracking what the solvers failed to see."
  - chapter: "002" — "A torn notebook page in unfamiliar handwriting is found on the floor inside the Achimota depot gate, using the same concise notation as the Lagos observation and recording Kwabena's wrong approach as the executor approach."
  - chapter: "004" — "A trace inside the gate room records three different solver approaches, the gap each missed, and the exact minute the condition was read clause by clause."
  - chapter: "005" — "The anchor's notebook logs all three wrong-approach solvers' entries side by side within the same gate event, each with precise register timestamps, before recording the correct approach — extending the simultaneous-approaches pattern first seen in chapter 004's trace."
...
### SOURCE: /cards/characters/char_001.json (full card)
id: "char_001"
name: "Yejide Adeyemi"
age: 0
...
first_appeared: 1
gate_history:
  - gate_id: "G-001", grade: 2, operation_encountered: "op_what_is_missing", approach_taken: "the executor", understood: true, looked_back: true, transferred_to_life: true
comprehension_state:
...
### SOURCE: /cards/locations/loc_lagos.json (full card)
id: "loc_lagos"
name: "Lagos, Nigeria"
first_appeared: 1
institutional_response: { primary_body: "", approach: "", failure_mode: "" }
...
### SOURCE: /core/concept_curriculum.md — Section 5 Arc Breakdown, Arc 1 row (arc_current = 1)
Arc: 1
Gate Grade: 1-2
Hard Operation: "Look at the unknown / what is missing (4)"
Easy Pairing: "Identify unknown / data / condition (2). Separate parts of condition (3)."
New Wrong Approach Type: "Executor. System builder. Information gatherer."
Narrative Engine: "First gates open. Society scrambling. Most survive Grade 1 by luck. Grade 2 begins killing those who got lucky on Grade 1."
...
contexts_demonstrated: ["family_domestic"]
...
Note: hidden_coherence field NOT read, per spec §CRITICAL RULES.
