...
| `assembled_prompt.md` fenced `ANCHOR_REQUIREMENT_JSON` (`anchor_appears`, `anchor_required_prose`) | Assembler, copying the discrete required phrases it chose for the anchor scene; emits `false` + `[]` when the pointer disables the anchor | `invoke_writer.py --check-prose`. Protects HARD RULE 8 before the paid Extractor call; an absent phrase emits `anchor_absent` and Orchestrator routes the whole-scene omission directly to `redo generation`. Evidence it fires: T-014 ch8 attempt-3 fixture fails while attempt 2 contains the exact phrase and passes this check |
