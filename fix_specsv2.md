...
- Do **not** touch `src/podcast_script_generator/llm/exceptions.py` — `PodcastError` stays as the domain base. `LLMError` is caught and re-raised as `ScriptGenerationError` at the `call_api` boundary; it never escapes into domain code.
