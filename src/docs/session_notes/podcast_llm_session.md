...
**Stack:** Python 3.10+, `pymupdf` (fitz), `anthropic` SDK. No frameworks.
...
**What it does:** Sends PDF text + prompt to Anthropic Claude API and returns the response string.
...
- Read API key from environment variable `ANTHROPIC_API_KEY` — do not hardcode
- Use model `claude-opus-4-5`
- Set `max_tokens` to `4096`
- Return only `response.content[0].text`
