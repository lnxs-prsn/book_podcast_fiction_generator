# Build Specs v6 — CLI / Endpoints / Engines Refactor

> Derived from `build_specsv5.md`.
> v6 hardens the spec against LLM-agent failure modes identified in a pass-by-pass risk
> review. No new features or structural changes — only defenses against high-probability
> execution failures.
>
> Changes from v5 are marked with **[v6]** in the relevant section.
> All prior version markers are retained where they were already correct.
>
...
|-----|----------|----------|
| Files modified in Pass 1.1 contain no comment explaining the missing `sys.path.insert` — agent will add it back when imports fail | High | Pass 1.1 |
| `run_local/run_llm/run_tts` contain references to module-level `SRC`/`ROOT` that must be renamed to `_SRC`/`_ROOT` after moving — spec says to rename but has no verification step | High | Pass 2.3 |
| `logging.basicConfig` added to `run_chapter.py` disappears when it is shimmed in Pass 5.3; `cli/podcast.py` never configures logging | Medium | Pass 2.5 |
| Pass 2.2 has no pre-flight check that no existing code uses `PodcastResult(ok=` (constructor form that breaks when `ok` becomes a property) | Low | Pass 2.2 |
| `__init__.py` files are listed in change tables but not verified in Done conditions | Low | Passes 2.1, 2.2 |
| `import re` required at module level in `endpoints/podcast.py` — mentioned in prose but never shown as an explicit code block | Low | Pass 2.3 |
| Pass 4.4 assumes `novel_pipeline` is installed but has no pre-condition to verify this before attempting the import | Medium | Pass 4.4 |
| Pass 5.3 pre-conditions do not verify that Pass 2.3 updated `run_book.py` to drop `import run_chapter as rc` | Medium | Pass 5.3 |
| Phase 6 smoke tests mix import verification and behavioral verification — an import failure inside a behavioral test raises instead of returning `PodcastResult`, making it look like a logic bug | Low | Phase 6 |
...
**[v6] Add a guard comment near the import in each of the four modified files** —
...
**[v6] Pre-flight check** — `ok` becomes a `@property` in this pass. If any existing code
...
**[v6] `import re` is listed explicitly in the module-level block above** — it is
required by the fiction-dir discovery code. Do not add it inside a function body.
...
**[v3] Helper function migration** — `run_chapter.py` currently defines three helper
functions (`run_local`, `run_llm`, `run_tts`) that the `main()` body calls. Pass 5.3 will
shim `run_chapter.py` to a single forwarding line, destroying its module namespace. If
these helpers are not moved now, the endpoint breaks at Pass 5.3.
...
helpers of the endpoint module.

**[v6] Path drift audit — perform before moving the functions**: Each of the three helpers
uses module-level path variables. In `run_chapter.py` these are named `SRC` and `ROOT`.
After moving, every occurrence of bare `SRC` and `ROOT` inside the function bodies must
be renamed to `_SRC` and `_ROOT` respectively (which are correctly anchored to
`endpoints/podcast.py`'s location). Audit with:

```
grep -n "\bSRC\b\|\bROOT\b" src/run_chapter.py
# identifies every reference inside run_local, run_llm, run_tts
```

For each hit inside one of these three functions: rename `SRC` → `_SRC`, `ROOT` → `_ROOT`.
After moving and renaming, verify no bare names remain in the endpoint file:

```
grep -n "\bSRC\b\|\bROOT\b" src/endpoints/podcast.py | grep -v "_SRC\|_ROOT\|# "
# must return empty
```
...
are function-scoped subdirectory inserts. After renaming `SRC`→`_SRC` and `ROOT`→`_ROOT`,
they resolve correctly in the new file location and move as-is.
...
grep -n "\bSRC\b\|\bROOT\b" src/endpoints/podcast.py | grep -v "_SRC\|_ROOT\|# "
# returns empty (no bare path variable names)

grep "_SRC\|_ROOT" src/endpoints/podcast.py
# returns hits confirming renamed constants are defined and used
```
...
**[v6] Logging configuration** — `run_chapter.py` currently configures `logging.basicConfig`
(added in Pass 1.3), but `run_chapter.py` is shimmed away in Pass 5.3. Add the same
...
**[v6] Pre-condition** — `novel_pipeline` must be importable before writing this file.
...
4. **[v6] `run_book.py` no longer references `run_chapter`**: Pass 2.3 added a temporary
   `from endpoints.podcast import run_llm, run_local, run_tts` to `run_book.py` and
   removed the `import run_chapter as rc` line. Verify this was done:
   ```
   grep "import run_chapter" src/run_book.py
   # must return empty before proceeding
   ```
   If this returns a hit, complete Pass 2.3's `run_book.py` update first.

> **Critical**: Apply BOTH shims in the same commit. If `run_chapter.py` is shimmed
> first alone, and Pass 2.3's `run_book.py` update was not completed (pre-condition 4
> above would have caught this), `run_book.py` would break. Pre-condition 4 prevents
> entering this pass in a state where that is possible.
...
### Pass 6.0 — Import Surface Verification

**[v6] Run before any behavioral pass.** A failure here means a package is missing or
...
"
# Must exit 0 before running passes 6.1–6.3.
...
Run only after Pass 6.0 succeeds. All checks should exit 0.
...
Run only after Pass 6.0 succeeds.
...
| `src/util/__init__.py` and `src/util/normalize.py` exist | Pass 2.3 | Pass 2.1 |
| `PodcastResult`, `ScriptMode` defined with correct types (`ok` as property) | Pass 2.3 | Pass 2.2 |
| `run_local/run_llm/run_tts` moved into `endpoints/podcast.py`; `SRC`/`ROOT` renamed to `_SRC`/`_ROOT` | Pass 5.3 pre-condition | Pass 2.3 |
| `run_book.py` updated to import helpers from `endpoints.podcast` (not `run_chapter`) | Pass 5.3 pre-condition 4 | Pass 2.3 |
...
| `logging.basicConfig` configured in `cli/podcast.py main()` | Pass 5.3 (shim removes it from run_chapter) | Pass 2.5 |
...
| `novel_pipeline` importable (egg installed) | Pass 4.4 | Pre-condition: `pip install -e .` |
...
| Pass 6.0 import surface verified | Passes 6.1–6.3 | Pass 6.0 |
