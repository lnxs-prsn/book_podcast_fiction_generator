# Build Specs v3 — CLI / Endpoints / Engines Refactor

> Derived from `build_specsv2.md` + `implementation_journalv2.md` (paper-run decisions).
> v3 closes spec gaps found during the paper run, makes all implicit requirements explicit,
> corrects broken Done conditions, and adds missing implementation detail.
>
> Changes from v2 are marked with **[v3]** in the relevant section.
>
...
|-----|----------|----------|
| `_load_config()` call sites need renaming — 3 in `tts/cli.py`, 4 in `call_api.py` (implied but unstated) | Low | Pass 1.1 |
| `sys.exit` Done condition for `main.py` grep catches the `__main__` guard line | Low | Pass 1.2 |
| `run_local/run_llm/run_tts` must move into `endpoints/podcast.py` — destroyed at Pass 5.3 if left in `run_chapter.py` | High | Pass 2.3 |
| `generate_book_podcast()` must raise on missing `toc_page` — never call `input()` | Medium | Pass 2.4 |
| `--slice-only` flag has no mapping to endpoint parameter | Medium | Pass 2.5 |
| `input()` Done condition grep catches `_prompt_yes_no` / `_prompt_choice` (unreachable under `auto_approve=True`) | Medium | Pass 4.2 |
| `q` quit path in `_prompt_user` callback lacks defined semantics | Low | Pass 4.3 |
| Pass 5.3 pre-condition must verify helpers were moved in Pass 2.3 | High | Pass 5.3 |
...
|------|--------|
| `src/config.py` | add |
| `src/run_chapter.py` | modify — remove `_load_config()` definition; add `from config import load_config` |
...
**[v3] Call-site renames** — removing the definition alone leaves broken calls:
...
| `src/tts/cli.py` | modify — add `import logging` + `logger`; convert 4 prints in `send_to_api()` to logger calls |
...
**`tts/cli.py` print mapping** (all inside `send_to_api()` at 8–12 spaces indentation):
- Submission confirmation → `logger.info(...)`
- Recovery-file notice → `logger.info(...)`
...
| File | Change |
|------|--------|
| `src/endpoints/podcast.py` | modify — add `generate_chapter_podcast()` **[v3] and move `run_local`, `run_llm`, `run_tts` helper functions** |
...
- Keep: context-file loading logic and fiction-dir discovery logic — they belong in the
  endpoint, not the CLI wrapper.
- Wrap entire body in:
  ```python
  try:
      ...
...
**[v3] `toc_page` handling** — `run_book.py`'s `main()` has an `input()` call to prompt for
`toc_page` when not provided. The endpoint must never call `input()`:
...
`dataclass` and `Callable` imports need to be added; `Path` is already imported.
...
3. Inside `_run_one_chapter()`: replace the `input("Approve and update living doc? [y/n/q]: ")`
   ...
   Map `True` → proceeds; `False` → rejected (loops for regeneration).
...
6. Return `SessionResult` at all exit points:
   - Early exit when `should_continue` is False →
     `SessionResult(chapters_written=0, final_chapter_number=0, cost_usd=0.0, completed=False, state_path=Path(config["state_file_path"]))`
   - Early exit when "Proceed?" is declined → same pattern
   - Normal completion →
     `SessionResult(chapters_written=completed, final_chapter_number=current_chapter, cost_usd=current_totals(config)["session_total"], completed=True, state_path=...)`

   Audit all `return` statements (minimum 3 early-exit points plus the fall-through) before
   marking this pass done.
...
3. **[v3] Helper functions have been moved**: `run_local`, `run_llm`, `run_tts` are in
   `endpoints/podcast.py`, NOT in `run_chapter.py`. This is what Decision 11 / Pass 2.3
   resolved. If they were not moved, shimming `run_chapter.py` breaks the endpoint.
   Verify:
   ```
...
| `_load_config()` call sites renamed | Pass 5.1 Done condition | Pass 1.1 |
